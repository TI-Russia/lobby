...
export default async function initChart() {
  ...
  const { data, lobby, lobby_level_0, myArrGroups, rawDep, rawRating, isSF } =
    await Data;
  ...
  function doTelling() {
    ...
    const nd = nodesWithClones
      .slice()
      .sort((a, b) => a.dupelganger - b.dupelganger)
      .sort((a, b) => a.id - b.id);
    ...
    function showCircles(state) {
      ...
      const highlightClass = (node) => {
        ...
        if (state === 3) {
          //силовики и крупный бизнес
          return node.group === 11551 || node.group === 11739
            ? "tellingCircle"
            : "tellingBluredCircle";
        }
        if (state !== 4) {
      ...
      let points = g_telling.selectAll("circle").data(dd, (d) => d.unq);
      ...
      points.exit().transition().delay(1000).remove();
      ...
      let labels = g_telling.selectAll("text").data(ll, (d) => d.text);
    ...
    div.addEventListener("scroll", function () {
      const witchOne = tops.find(
        (d) =>
          d.offsetTop - div.getBoundingClientRect().height + 100 < div.scrollTop
      );
      if (witchOne) {
  ...
  function composeNodes() {
    svg = d3.select("#clusters").append("svg").attr("id", "chart").append("g");
    SetupSVG();
    ...
    nodes = data.reduce(function (acc, d) {
      ...
      if (group.level == 1)
        clusterParentId = lobby.find((x) => x.id == group.parent).id;
      if (group.level == 2) {
        clusterParentId = lobby.find(
          (x) => x.id == lobby.find((x) => x.id == group.parent).parent
        ).id;
    ...
    nodes.forEach(function (node) {
      ...
      const clones = nodes.filter(
        (x) => x.id == node.id && x.cluster == node.cluster
      );
      if (clones.length > 1) {
  ...
  function doChart() {
    ...
    const yScale = d3.scaleLinear().domain([0, rows]).range([100, maxY]);
    ...
    zoom_handler
      ...
      .translateExtent([
        [extentXMin, extentYMin],
        [extentXMax, extentYMax],
      ])
      .extent([
        [extentXMin, extentYMin],
        [extentXMax, extentYMax],
    ...
    function showDetail(d) {
      ...
      if (Array.isArray(d.cloneClusters)) {
        cloneclustersNames = d.cloneClusters.map((x) =>
          lobby.find((y) => y.id == x)
        );
      }
      ...
      if (isSF) {
        content += `<span class="value">${FRACTIONS[d.fraction].name}, ${
          d.total_years
        } ${Pluralization(
          d.total_years,
          "год в СФ",
      ...
      if (cloneclustersNames != "") {
        content += `<span class="value">${cloneclustersNames.map(
          (name) => name.name
        )}</span><br/>`;
      }
    ...
    function MakeSelect() {
      createSelect("select_committees", "Комитет по", "committees");
      createSelect(
        "select_position_gd",
        "Должность в ГД",
        "positions",
      ...
      const tree = tree_func.list_to_tree(
        lobby.sort((a, b) => a.order - b.order)
      );
      ...
      d3.selectAll(".buttons button").on("click", function () {
        ...
        if (d3.select(this).classed("is-active")) {
          if (d3.select(this.parentNode).attr("id") != "fraction") {
            ...
            if (
              d3.select(this.parentNode).selectAll("button.is-active").size() >
              1
            ) {
      ...
      function createSelect(selector, placeholder, key, map) {
        ...
        select
          ...
          .text((d) => {
            const text =
              key == "committees" ? d && d.replace("Комитет ГД по", "") : d;
      ...
      function ZoomeToLobby(active) {
        ...
        if (!active_enclose.node()) {
          const lob = nodes.find(
            (x) => x.cluster == active || x.clusterMin == active
          );
        ...
        svg1.transition().duration(450).call(zoom_handler.transform, transform);
      }
      ...
      function onchange(init) {
        ...
        if (isSF) {
          ...
          s_position_gd_raw = d3
            .select("select#select_position_gd")
            .property("value");
          s_position_gd =
            s_position_gd_raw &&
            Number(findKey(POSITIONS, (val) => val === s_position_gd_raw));
        }
        ...
        let result = circles.filter(
          (x) =>
            ...
            (!isSF
              ? +x.convocations.length >= +r_conv[0] &&
                +x.convocations.length <= +r_conv[1]
              : true) &&
            (isSF
              ? +x.total_years >= +r_conv[0] && +x.total_years <= +r_conv[1]
              : true) &&
            +x.age >= +r_age[0] &&
            +x.age <= +r_age[1] &&
            (b_fraction
              ? b_fraction == x.fraction || b_fraction == x.fraction_sf
              : true) &&
      ...
      function MakeAutoComplete() {
        ...
        new Awesomplete(input, { list: rawDep.map((x) => x.fullname) });
        ...
        d3.selectAll("#presets-first a")
          ...
          .forEach(function (link) {
            ...
            link.addEventListener("click", function (e) {
              ...
              if (power) {
                d3.select(`#power button[value="${power}"]`).classed(
                  "is-active",
                  true
                );
              }
      ...
      function hightlightOn(spot) {
        ...
        const transition_fadeout = d3
          .transition()
          .duration(10)
          .ease(d3.easeLinear);

        circles
          .classed((d) => d.color, true)
          .transition()
          .style("opacity", 0.2)
          .on("end", () => {
            i--;
            if (i === 0 && --count > 0) {
              ...
              const temp = clearClusters.filter((x) =>
                circles_clusters.includes(x.cluster)
              );
              const temp2 = ShowedClusters(temp, k, isSF).map((s) => s.cluster);

              const labels_spot = labels.filter((x) =>
                temp2.includes(x.cluster)
              );
